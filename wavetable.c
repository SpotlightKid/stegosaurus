...
const int wavetable_number_of_waves = 8;
wave* waves[8];
...
float wavetable_tick(oscillator* osc) {
    ...
    if (index > length - 1 ) {
        ...
        if (tst<0) tst = 0;
        if (tst>7) tst = 7;
        osc->bandlimit_offset = 0;  // (tst*4410);
    }
...
void wavetable_initialise() {
    ...
    f=0;
    pc=0;
    q=0;    // filter coefficients
    bf0=0;
    bf1=0;
    bf2=0;
    bf3=0;
    bf4=0;  // filter buffers (beware denormals!)
    t1=0;
    t2=0;   // temporary buffers
