...
def fixup_soname(self):
    if not getattr(self, 'lv2_bundle', False):
        return

    self.env['cxxshlib_PATTERN'] = self.env['cshlib_PATTERN'] = self.env['pluginlib_PATTERN']
...
def create_task_lv2plugin(self):
    ...
    # If lv2_target is set, the value there will be used for the bundle name
