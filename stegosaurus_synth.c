...
void stegosaurus_synth_init(int sample_rate) {
    ...
    for (int v=0; v<4; v++) {
        ...
        voices[v]->buf0_left=0;
        voices[v]->buf1_left=0;

        voices[v]->bf0_left=0;
        voices[v]->bf1_left=0;
        voices[v]->bf2_left=0;
        voices[v]->bf3_left=0;
        voices[v]->bf4_left=0;  // filter buffers (beware denormals!)
        voices[v]->t1_left=0;
        voices[v]->t2_left=0;   // temporary buffers

        voices[v]->selectivity_left=90;
        voices[v]->gain1_left=0;
        voices[v]->gain2_left=0.25;
        voices[v]->ratio_left=7;
        voices[v]->cap_left=0;

        // Init filter right

        voices[v]->buf0_right=0;
        voices[v]->buf1_right=0;

        voices[v]->f_right=0;
        voices[v]->pc_right=0;
        voices[v]->q_right=0;    // filter coefficients
        voices[v]->bf0_right=0;
        voices[v]->bf1_right=0;
        voices[v]->bf2_right=0;
        voices[v]->bf3_right=0;
        voices[v]->bf4_right=0;  // filter buffers (beware denormals!)
        voices[v]->t1_right=0;
        voices[v]->t2_right=0;   // temporary buffers

        voices[v]->selectivity_right=90;
        voices[v]->gain1_right=0;
        voices[v]->gain2_right=0.25;
        voices[v]->ratio_right=7;
        voices[v]->cap_right=0;
...
void stegosaurus_synth_play(float* const out_left, float* const out_right, uint32_t n_frames) {
    ...
    for (uint32_t fr=0; fr<n_frames; fr++) {
        float out = 0;
